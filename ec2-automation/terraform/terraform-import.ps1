...
if ($sgId -ne "None" -and $sgId -ne "")
{
    ...
    terraform import -var-files="dev.tfvars" aws_security_group.ec2_sg $sgId
}
else 
{
...
$role = aws iam get-role --role-name $roleName --region $region 2>$null

if ($LASTEXITCODE -eq 0)
{
    Write-Output "[OK] IAM Role '$roleName' exists"
    Write-Output "[INFO] Importing IAM Role into Terraform state..."
    terraform import -var-files="dev.tfvars" aws_iam_role.ec2_s3_role $roleName
}
else
{
